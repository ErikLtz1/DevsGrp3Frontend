...
function PlayingField(props: Props) {
  ...
  useEffect(() => {
    if (props.stompClient) {
      ...
      const bulletSubscription = props.stompClient?.subscribe(
        "/destroy/bullets",
        (message) => {
      ...
      const playerDisplaySubscription = props.stompClient.subscribe(
        "/destroy/player-registration",
        (message) => {
      ...
      const newGameSubscription = props.stompClient?.subscribe(
        "/destroy/new-game",
        (message) => {
      ...
      const scoreSubscription = props.stompClient.subscribe(
        "/destroy/player-scores",
        (message) => {
          setPlayers(JSON.parse(message.body));
        }
      );
      const finalScoreConfirmationSubscription = props.stompClient?.subscribe(
        "/destroy/final-score-confirmation",
        (message) => {
  ...
  useEffect(() => {
    ...
    if (winner !== "") {
      startConfetti();
      setTimeout(() => {
        stopConfetti(), setWinner(""), newGame();
      }, 7000);
    }
  }, [winner]);
  ...
  function getColour(x: number, y: number) {
    if (players.length === 4) {
      const colour = players.find(
        (player) => player.x === x && player.y === y && player.active === true
      )?.colour;
      ...
      if (colour) {
        ...
        players.find(
          (player) =>
            player.x === x && player.y === y && player.active === false
        )
      ) {
        return {
  ...
  function roundEnd() {
    if (roundNumber < 4) {
      ...
      if (props.stompClient) {
        ...
        const newRoundSubscription = props.stompClient.subscribe(
          "/destroy/round-start-confirmation",
          (message) => {
  ...
  return (
    <div className="playingFieldOuterDiv">
      ...
      {gameStart && roundNumber < 5 ? (
        <div className="countdown">
          <h2>
            {count > 0
              ? "Round " + roundNumber + "\nbegins in " + count + " seconds."
              : "Destroy!"}
          </h2>
      ...
      {winner !== "" ? (
        <div className="winnerDiv">
          <h1 className="winner">
            {"The Winner is " +
              winner +
              "\n A New game will start when all players are ready!"}
          </h1>{" "}
        </div>
      ) : null}
